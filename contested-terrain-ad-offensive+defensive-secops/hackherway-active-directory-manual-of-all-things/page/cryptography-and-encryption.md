...
![A book cover with palm trees

Description automatically generated](<../.gitbook/assets/0 (20).png>)

|                                                       | <h3>CHAPTER</h3> |
| ----------------------------------------------------- | ---------------- |
| <h3><strong>CRYPTOGRAPHY AND ENCRYPTION</strong></h3> | 20               |

This chapter aims to provide a foundational understanding of cryptographic concepts and terminology to help novice hackers navigate and utilize cryptography and encryption in their activities.

### Introduction

As hackers, we often encounter the challenge of cryptography and encryption. Every hacker, security professional, system administrator and security engineer understand that encryption significantly complicates the task for hackers and attackers. Sometimes, hackers might even find encryption useful for concealing their own actions and messages.

Many applications and protocols employ some form of encryption, or other, to ensure the confidentiality, integrity, authenticity and nonrepudiation of data. To crack passwords and encrypted protocols such as SSL and wireless networks, you need a basic understanding of cryptographic systems and encryption algorithms, concepts, and terminology.

For many new hackers, the concepts and terminology of cryptography can be daunting, obscure, and downright overwhelming. To begin with, cryptography is the science and art of hiding messages to keep them confidential and then revealing them so only the intended recipient can read them. In essence, cryptography is the science of _secret messaging._

This chapter aims to demystify cryptography for newcomers and provide some clarity on the subject. This chapter is geared towards a more cursory introduction to cryptography for novice hackers, and not an in-depth exploration of encryption algorithms and mathematical, quadradic and polynomial concepts and foundations that usually accompany key encryption algorithms. This chapter will introduce you to the basic terms and concepts so that when you encounter discussions on hashing, wireless cracking, or password hacking involving encryption technologies, you will have a foundational understanding of the topic.

To be clear, the goal here is not to turn you into an expert cryptographer (that would take years and is beyond the scope of this chapter and book), but to help you become familiar with the terms and concepts of everyday cryptography, aiding you in becoming a competent hacker. I will use as much plain English as possible to explain these technologies, but like everything in IT, cryptography and encryption have a specialized vocabulary. Terms like _cipher, plaintext, ciphertext, keyspace, nonce, block size,_ and _collisions_ can make studying cryptography confusing and overwhelming for most beginners. Some terms like “collision,” are unique and don’t have a straightforward replacement in plain English.

Let us break into this chapter by starting with an introduction to cryptography and encryption concepts and fundamentals and we investigate its history and roots of origin.

### **Cryptography History and Roots of Origin**

_The_ term _“Cryptography”_ originates from the Greek words _“kryptos,”_ meaning “hidden or secret,” and _“graphia,”_ meaning “writing;” therefore, cryptography is essentially _“the secret art of writing.”_ In the information security and cybersecurity industry, understanding and having a firm grasp of the subject matter, cryptography is fundamental to comprehending the myriad of related technologies that are intertwined, combined, implemented, or integrated with encryption.

To start, understanding cryptography, especially if you’re a beginner hacker, is crucial for the offensive security/red team/adversarial part of you, for several reasons:

1. **Exploiting Weaknesses:** It’s important to understand the need to identify and exploit weaknesses in cryptographic implementations. Knowledge of cryptography helps us to understand common flaws in vulnerabilities, such as weak encryption algorithms, poor key management, or implementation and misconfiguration errors.
2. **Bypassing Security Measures:** There will come a time when you’ll find yourself simulating real-world attack scenarios and simulations that real-world adversaries might use. Many sophisticated hacks and attacks involve breaking or circumventing encryption, if you think about it. Understanding cryptographic methods allows us to create realistic attack scenarios that can test an organization’s defenses against such threats.
3. **Reverse Engineering:** Cryptography is used in various security mechanisms, including Digital Rights Management (DRM), secure communications, and software protection. We often have the need to reverse engineer these systems to understand their encryption methods and find different ways to exploit, bypass, obfuscate, or evade such detections.
4. **Data Exfiltration:** In offensive operations, we might need to exfiltrate data from a compromised system. This includes data exfiltration in defensive security, as well. Understanding encryption allows us to decrypt sensitive information and use it for further exploitation or to demonstrate the impact of a breach.
5. **Creating Secure Backdoors:** It’s always good to have a Plan B; therefore, it’s always good to know how to properly implant a backdoor in systems during operations. To avoid detection and ensure our persistence, we need to encrypt these backdoors or the ingress/egress communications between compromised systems, bots, and botnets, and Command and Control (C2) servers.
6. **Evading Detection:** Offensive security operations require stealth. Using encryption, we can hide our activities, encrypt malicious payloads, and secure communications channels to avoid detection by defensive security measures.

Cryptography and encryption knowledge empower us to more effectively mimic real-world adversaries, identify, and exploit vulnerabilities, while enhancing the realism and effectiveness of our overall assessments.
...
During Julius Caesar’s time, historians documented that the future emperor could encode and decode messages using his system entirely in his head. Although the system was quite simple by today’s standards, this was an astonishing feat for that era.
...
### Cryptographic Fundamentals – The Purpose of Cryptography

\
The purpose of cryptography is to provide confidentiality, integrity, authentication and nonrepudiation of data. In doing so, confidentiality protects data by making it unreadable to all but authorized entities, integrity protects data from accidental or deliberate manipulation by entities, authentication ensures that an entity is who they claim to be, and nonrepudiation provides proof that an entity performed a particular action or function.
...
Encryption, or what’s known as _**“data at rest,”**_ can be used to protect sensitive or classified data stored on information technology equipment (IT) and media. In addition, encryption of _**data in transit**_ can be used to protect sensitive or classified data communicated over public network infrastructure; however, when an organization uses encryption for data at rest, they are simply reducing the immediate consequences of data being accessed by unauthorized entities.

### International Standards for Cryptographic Modules

The International Organization for Standardization (ISO)/International Electrotechnical Commission (IEC) 19790:2012, “_Information technology – Security techniques – Security requirements for cryptographic modules,”_ and ISO/IEC 24759:2017, _“Information technology – Security techniques – Test requirements for cryptographic modules,”_ are international standards and methodologies for the design and validation of hardware and software cryptographic modules.

Federal Information Processing Standard (FIPS) 140-3, _“Security Requirements for Cryptographic Modules,”_ and National Institute of Standards and Technology (NIST) Special Publication (SP) 189-140, _“FIPS 140-3 Derived Test Requirements (DTR): CMVP Validation Authority Updates to ISO/IEC 24759,”_ are United States standards based upon ISO.IEC 19790:2012 and ISO/IEC 24759:2017.

### Can Crypto Keys Be Hacked?

\
Hackers can break encryption to access the data using several different methods. The most common method is stealing the encryption key itself. Another common way is intercepting the data either before it has been encrypted (data at rest) by the sender or after it has been decrypted by the recipient.

### How are Encryption Keys Transmitted?
...
\
Cryptographic systems rely on random numbers for generating keys and other operations. If the random number generation is flawed or predictable, you can exploit this weakness to compromise the cryptographic system. One of the most straightforward yet resource-intensive methods used to break encryption is a brute-force attack. In this method, you systematically try every damn possible combination of keys until you find the correct one and can decrypt the ciphertext.

### Hacking Cryptographic Keys via Pythonic Brute-Force

\
To conduct an attack demonstration of how cryptographic keys can be exploited via a brute-force attack, it’s crucial to have a clear understanding of the target and the constraints of the demonstration. Brute-force attacks involve systematically trying all possible combinations until the correct key is found. For cryptographic purposes, this can be computationally expansive and time-consuming, especially for strong encryption algorithms.

However, below is a Python example that demonstrates a brute-force attack on a simple Caesar substitution cipher. This example is intentionally simplistic to keep the focus on the concept of brute-forcing rather than the complexities of modern cryptography.
...
Description automatically generated](<../.gitbook/assets/9 (8).png>)\
...
Description automatically generated](<../.gitbook/assets/10 (8).png>)\
_**FIGURE X:** Symmetric encryption process._
...
Description automatically generated](<../.gitbook/assets/12 (7).png>)
...
Description automatically generated](<../.gitbook/assets/13 (7).png>)
...
Description automatically generated](<../.gitbook/assets/14 (7).png>)
...
Description automatically generated](<../.gitbook/assets/15 (7).png>)\
...
Description automatically generated](<../.gitbook/assets/17 (6).png>)
...
* Published in 1976 by Whitfield Diffie and Martin Hellman, this method of digital encryption was one of the first practical examples of public key cryptography. Diffie-Helman key exchange is a method of digital encryption that securely exchanges cryptographic keys between two parties over a public communications channel without their conversation being transmitted over the internet. The two parties use symmetric cryptography to encrypt and decrypt their messages.

Diffie-Hellman key exchange raises numbers to a selected power to produce decryption keys. The components of the keys are never directly transmitted, making the task of a would-be code breaker mathematically overwhelming. The method doesn’t share information during the key exchange, per se, and the two parties have no prior knowledge of each other; however, together, the two parties create their key in unison.

Uses of the Diffie-Hellman Key Exchange

Diffie-Hellman key exchange’s goal is to securely establish a channel to create and share a key for symmetric key algorithms. Generally, it is used for encryption, password-authenticated key agreement and forward security. Password-authenticated key agreements are used to prevent Man-in-the-Middle (MiTM) attacks. _**Forward secrecy-based protocols**_ protect against the compromising of keys by generating new key pairs for each session.

Diffie-Hellman key exchange is commonly found in protocols, such as Transport Layer Security (TLS), Secure Shell (SSH), and Internet Protocol (IP) Security (IPSec). For example, in IPSec, the encryption method is used for key generation and key rotation.

Even though Diffie-Hellman key exchange can be used for establishing both public and private keys, the Rivest-Shamir-Adelman (RSA) algorithm can also be used, since it’s able to sign public key certificates.

Diffie-Hellman Operations

The Diffie-Hellman key exchange is a process that allows two people to create a shared secret over an insecure communications channel. They start by agreeing on a large prime number and a base number. Each person then picks a private number, which they keep to themselves. Using the agreed-upon numbers, they create a public value from their private number and exchange these public values. With the public value they receive and their private number, each person can compute a shared secret key. This method ensures that even if someone sees the public values, they can’t and won’t be able to figure out the shared secret, making it secure for further encrypted communications.

![](<../.gitbook/assets/18 (6).png>)

### Exploiting Diffie-Hellman

There are numerous ways in which we can exploit Diffie-Hellman Key Exchange (DHKE) vulnerabilities. Some of the techniques we may employ include:

1. **Man-in-the-Middle (MiTM) Attacks**
   * DHKE is vulnerable to MiTM attacks, where we actively intercept communications between two parties (A and B, respectively). We can use masquerading as B to A and as A to B, allowing us to eavesdrop on the exchanged keys or manipulate the communications in our favor.
2. **Exploit Kits:**
   * Exploit kits (EKs) are malicious tools available that can be leveraged to deliver malware. In a notable case, the Angler EK utilized the Diffie-Hellman protocol to prevent detection. By implementing DHKE, we can ensure that firewalls cannot decipher shellcodes and exploits by analyzing intercepted traffic.
3. **Logjam Vulnerability:**
   * The Logjam vulnerability affected DHKE by exploiting this weakness using MiTM techniques to force a downgrade to a 512bit export-grade cipher. Successful exploitation allowed for the recovery of session keys and modification of traffic contents.
4. **Stegano EK Adoption:**
   * The Stegano (Astrum) exploit kit adopted the Diffie-Hellman algorithm to hinder analysis. This move aimed to make it more challenging for security researchers to analyze and detect the kit’s malicious payloads.
5. **Current State:**
   * While DHKE has faced vulnerabilities, everyday hackers and attackers may not commonly exploit it. Researchers suggest that it would take significant computational resources to break DHKE in practice.

### **Diffie-Hellman Exploitation**

In this section, we will explore some Diffie-Hellman exploitation scenarios. First, we’ll start off with simulating a Diffie-Hellman key exchange in Python:

### **EL Gamal**

* A hybrid algorithm that uses asymmetric keys to encrypt the symmetric key, which is then used to encrypt the rest of a message. Based on the Diffie-Hellman algorithm, it also relies on discrete logarithms.

### **RSA (Rivest-Shamir-Adelman)**

* Patented in 1977, though RSA symbolically released its patent to the public about 48 hours before it expired in 2002. RSA is still used in various applications and processes, such as e-commerce and comparable applications. In general, this algorithm is no longer used quite so much because of its performance and overhead, and as a result it has been replaced with newer algorithms. RSA is based on the difficult problem of factoring two large primes (like the previous calculation exercise).

### **Elliptic Curve Cryptography (ECC)**

* This process is based on the difficulty of solving the elliptic curve discrete logarithm problem (which you won’t have to think about here). Because the algorithm is so computationally intensive, shorter key lengths offer better security relative to other algorithms using the same key length. These shorter keys require less power and memory to operate, which means ECC may be used more often on mobile devices with less processor or battery power.

FYI – FOR YOUR INFORMATION

Dr. Whitfield Diffie and Dr. Martin E. Hellman were the first to publish their concept of public key protocol exchange, properly named the Diffie-Hellman Key Exchange (DHKE) protocol in 1976.

The main strength of asymmetric encryption is that it addresses the most serious problem with symmetric encryption: _**key**_ _**distribution**_. Although symmetric encryption uses the same key to encrypt and decrypt data, asymmetric encryption uses two related, but different keys that can reverse whatever operation the other platforms. Because of the unique properties that are a characteristic of asymmetric encryption, simply having one key can be placed in a location that is accessible to anyone who may need to send information to the recipient, who has the corresponding private key. Someone can safely distribute the public key and not worry about compromising security in any way. The public key can be used by anyone needing to send a message to the owner of the public key (and private key) because once the public key is used to encrypt a message, it cannot be used to decrypt that message. Thus, there is no fear of unauthorized key disclosure. When a message is delivered, the receiver decrypts it with the private key.
...
Asymmetric encryption can employ _**trapdoor functions,**_ which are functions that are easy to compute in one direction but tough to compute in the other direction.
...
| ------------------------------- | ------------------------------------------------------ | ----------------------------------------------------------------------------- |
| **Number of keys**              | <p><br>One key shared by two or more parties</p>       | Pairs of keys                                                                 |
...
_**TABLE X:** Feature comparison between asymmetric and symmetric encryption._
...
_**FIGURE X:** Basic principle of how hashing algorithms work._

### COMPONENTS OF HASHING

![What is Hashing? - GeeksforGeeks](<../.gitbook/assets/20 (3).png>)
...
* **Overview:** An older one-way hash function used in the Privacy-Enhanced Mail (PEM) protocols along with MD5. It produces a 128bit hash value for an arbitrary input. It is similar in structure to MD4 and MD5 but is slower and less secure.
* **Characteristics:** Structured like MD4 and MD5, MD2 is slower and less secure. Its primary use was in email encryption, but it has largely been superseded by more secure alternatives, such as:
...
_**FIGURE X:** Illustration depicting a collision attack against hash functions._
...
* **MD5 Decoder:** Offers various tools including an MD5 decoder for decrypting MD5 hashes.

_(**Source:**_ [_https://www.dcode.fr_](https://www.dcode.fr/)_)_

* **CrackStation:** Provides a comprehensive password cracking service with a large database for decrypting hashes.

_(**Source:**_ [_https://crackstation.net_](https://crackstation.net/)_)_

* **Md5 Decrypt & Encrypt:** Allows users to encrypt and decrypt MD5 hashes using a vast precomputed hash database.

_(**Source:**_ [_https://md5decrypt.net_](https://md5decrypt.net/)_)_

* **md5hashing:** Offers tools for MD5 encryption, decryption, and other algorithms.

_(**Source:**_ [_https://md5hashing.net_](https://md5hashing.net/)_)_

* **MD5 Encrypt/Decrypt:** Provides a simple interface for MD5 encryption and decryption.

_(**Source:**_ [_https://10015.io.com_](https://10015.io.com/)_)_

* **MD5 Decryption:** A service dedicated to decrypting MD5 hashes uses an extensive hash database.

_(**Source:**_ [_https://www.md5online.com_](https://www.md5online.com/)_)_

* **MD5Decrypter:** Offers MD5 hash decryption services, leveraging a large database of precomputed hashes.

_(**Source:**_ [_https://www.md5decrypter.com_](https://www.md5decrypter.com/)_)_

* **OnlineHashCrack:** Provides online password cracking services including MD5 hash decryption.

_(**Source:**_ [_https://www.onlinehashcrack.com_](https://www.onlinehashcrack.com/)_)_

* **HashKiller.io:** Community-driven hash cracking service with a vast database of cracked hashes.

_(**Source:**_ [_https://hashkiller.io_](https://hashkiller.io/)_)_

* **Md5.My-Addr.com:** Offers tools for decoding MD5 hashes using a precomputed hash database.

_(**Source:**_ [_https://md5.my-addr.com_](https://md5.my-addr.com/)_)_

* **cmd5:** Provides MD5 decryption services, supporting various hash types.\
  _(**Source:**_ [_https://www.cmd5.org_](https://www.cmd5.org/)_)_
* **Decrypt MD5 Hash:** Online tool for decrypting MD5 hashes using a large database.

_(**Source:**_ [_https://hashtoolkit.com_](https://hashtoolkit.com/)_)_

* **Online MD5 Hashed Validator:** Validates and decodes MD5 hashes online.

_(**Source:**_ [_https://www.javainuse.com_](https://www.javainuse.com/)_)_

* **Decode MD5 Hash:** Provides MD5 hash decoding services.

_(**Source:**_ [_https://md5.web-max.co_](https://md5.web-max.co/)_)_

* **MD5 Decoder Tool:** Tool for decoding MD5 hashes, like md5.my-addr.com.

_(**Source:**_ [_http://md5.my-addr.com_](http://md5.my-addr.com/)_)_<br>
